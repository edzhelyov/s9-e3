...
Locatable = {
    ...
    freeInput: function() {
        ...
        this.nextInput++;
        i.attr('fill', 'black');
        i.attr('data-connected', 'true');
        return {'x': x, 'y': y};
    },
    inputSources: function() {
        var sources = [],
            circle_inputs = this.svg.find('.input'),
            self = this;

        circle_inputs.each(function(idx, item) {
            if ($(item).attr('data-connected') == 'true') {
                sources.push(self.connections.inputs[idx]);
            } else {
                sources.push($(item).attr('data-source'));
            }
        });
...
Element = function(svg_element) {
    ...
    svg_element.find('.input').click(function() {
        if ($(this).attr('data-connected') == 'true') return;

        var s = $(this).attr('data-source');

        if (s == 'on') {
            $(this).attr({fill: 'black', 'data-source': 'off'});
        } else {
            $(this).attr({fill: 'yellow', 'data-source': 'on'});
        }
    });
